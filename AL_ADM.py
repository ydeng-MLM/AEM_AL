...
if __name__ == '__main__':   
    for i in range(10):
        data_types = ['ADM']
        query_types = ['GS', 'random']
        for data_type in data_types:
            for query_type in query_types:
                for n_init in [5000]:
                    for pool_ratio in [1000]:
                        n_pool = pool_ratio
                        n_batch = 1000
                        n_com = 1
                        rd = 1
                        ...
                        #np.savetxt('benchmark_summary/ADM_opt/062122/'+data_type+'_mse_array_n_init_'+str(n_init)+'_pool_ratio_'+str(pool_ratio)+'_'+query_type+str(i)+'.csv', mse_array, delimiter=',')
