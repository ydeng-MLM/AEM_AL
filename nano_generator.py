...
def toy_initial(n_sample):
    x = np.random.uniform(-10, 10, [n_sample, 1]).astype('float32')
    y = toy_func(x).astype('float32')
    return x, y

def toy_func(x):
    return (np.sin(x)*x**3)/1000
...
def nano_generate(low_bound, up_bound, num_samples, num_layers):
    data_y = []
    data_x = []
