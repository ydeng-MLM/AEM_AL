...
import pandas as pd
# Own module
from NA.utils.time_recorder import time_keeper
from NA.utils.helper_functions import simulator
...
class Network(object):
    ...
    def simulate(self, X, load_state_dict=None):
        """
        The prediction function, takes Xpred file and write Ypred file using trained model
        :param X: Normalized X {-1, 1} geometry for ADM dataset
        :param load_state_dict: If None, load model using self.load() (default way), If a dir, load state_dict from that dir
        :return: Y: Ensemble model predicted
        """
        if load_state_dict is None:
            self.load()  # load the model in the usual way
        else:
        ...
        # Get small chunks for the evaluation
        chunk_size = 100
        Ypred_mat = np.zeros([len(X_tensor), 2000])
        for i in range(int(np.floor(len(X_tensor) / chunk_size))):
            Xpred = X_tensor[i * chunk_size:(i + 1) * chunk_size, :]
            if cuda:
                Xpred = Xpred.cuda()
            Ypred = self.model(Xpred).cpu().data.numpy()
            Ypred_mat[i * chunk_size:(i + 1) * chunk_size, :] = Ypred
