...
class GaussianNoise(torch.nn.Module, AbstractNoiseModel):
    ...
    def __init__(self, cuda=False):
        ...
        self.cuda = cuda
        self.const = torch.log(torch.FloatTensor([2 * math.pi])).to('cuda')
    ...
    def loss(self, y_true=None, mu_pred=None, log_var_pred=None, reduce_mean=True):
        ...
        #print(var_pred.shape, mu_pred.shape, y_true.shape)
        var_pred = var_pred.mean(dim=0)
        mu_pred = mu_pred.mean(dim=0)
